...
3. If auth is missing entirely, offer the "One-Time User Setup" section. **If the user rejects API setup** (explicit "I'm not paying for X" or "I don't want developer bullshit"), pivot immediately to `references/fallback-no-auth.md`. Do not push the API approach a second time.
   - For public metadata: use the `__INITIAL_STATE__` curl scrape
   - For auth-walled content (articles, threads): offer cookie-based browser auth
...
| `CreditsDepleted` | $0 balance on X API, or on free tier (write-only — no GET credits) | Buy credits (min $5) in Developer Console → Billing. If on free tier: reading requires Basic ($100/mo) or Pro ($5K/mo). Free tier is POST-only. Use Firecrawl/nitter for reading, xurl for writing. |
| 401 on whoami but OAuth 1.0a creds are set | consumer_key copied as the numeric user ID instead of the actual API Key | The consumer key is the app's API Key (alphanumeric, ~25 chars), NOT your numeric user ID. The access token starts with the numeric user ID. They're different values on the same "Keys and tokens" page. |
...
- **Free tier:** OAuth 1.0a only. Write-only — POST tweets, replies, likes, follows work. GET operations (timeline, search, user tweets) return `CreditsDepleted` — there are NO read credits on free. For reading, use Basic ($100/mo, 10K GET reads) or Pro ($5K/mo, search + 1M reads). Or scrape via Firecrawl/nitter — no API cost, profile pages are public.
- **OAuth 1.0a manual config:** Free-tier apps don't use the OAuth 2.0 PKCE flow. Instead, get all four token values from the X Developer dashboard ("Keys and tokens" page) and write them directly to `~/.xurl`:
  ```yaml
  apps:
      default:
          client_id: ""
  ...
  default_app: default
  ```
  OAuth 1.0a tokens are permanent — no refresh, no expiry. The consumer key is NOT the same as the access token; getting them crossed is a common failure.
