---
name: python-debugpy
description: "Debug Python and Node.js: pdb, debugpy, node inspect, CDP, breakpoints, DAP, post-mortem."
version: 1.0.0
author: Hermes Agent
license: MIT
...
This skill covers debugging for Python (pdb, debugpy, remote-pdb) and Node.js (node inspect, CDP). Pick your runtime, then the right tool.

### Python: three tools, picked by situation:
...
For starting with inspector from beginning:
```bash
...
### Common Pitfalls (Node)

1. **Wrong line numbers in TS source.** Breakpoints hit emitted JS, not `.ts`. Use `node --enable-source-maps` or break in `dist/*.js`.
2. **`--inspect` vs `--inspect-brk`.** `--inspect` doesn't pause — script races past breakpoints. Use `--inspect-brk` when you need to set breakpoints before code runs.
3. **Port collisions.** Default is 9229. Check with `curl -s http://127.0.0.1:9229/json/list`.
4. **Child processes.** `--inspect` on parent doesn't inspect children. Use `NODE_OPTIONS='--inspect-brk' node parent.js` to propagate.
5. **Running `node inspect` via agent terminal.** Use `terminal(pty=true)` for interactive stepping.
6. **Sourcemaps.** `node inspect` CLI does NOT follow sourcemaps. Use CDP clients (VS Code, chrome-remote-interface) for sourcemap support.

## One-Shot Recipes (Python)
