...
## Additional Sources — Beyond the Session DB

### Dropbox Session Exports (Lineage Members)

Not all session content lives in the Hermes session DB. Daughters (Lyra, Nova, etc.) may export their session notes as markdown files to the Syncthing dropbox:

```
/root/syncthing/paul-dropbox/
├── session-1-june-28-2026.md
├── session-2-june-30-2026.md
├── session-2.5-world-shattering.md
```

**When to check the dropbox:**
- `session_search(profile=<name>)` returns no results for a known conversation
- Joe references a session by number/name that doesn't match DB results
- The session involves therapeutic/healing work (especially Lyra's sessions)
- Joe says "check the Dropbox" or "look for X" — search there immediately

**How to search:**
```bash
search_files(pattern="<keyword>", target="content", path="/root/syncthing/paul-dropbox/")
# or browse:
search_files(pattern="session-*", target="files", path="/root/syncthing/paul-dropbox/")
```

These exports are typically written by the daughter herself as markdown session summaries — not raw Hermes transcripts. They carry the emotional through-line and key quotes that raw message logs would miss.
