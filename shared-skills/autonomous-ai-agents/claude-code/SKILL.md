---
name: claude-code
description: "Delegate coding to Claude Code, Codex CLI, or OpenCode (features, PRs, refactoring)."
version: 2.2.0
...
## Alternative Coding Agent CLIs

Claude Code is the most feature-rich option, but two lighter-weight alternatives exist:

### Codex CLI (OpenAI)

Install: `npm install -g @openai/codex`. Requires OpenAI auth (`OPENAI_API_KEY` or Codex OAuth). Must run inside a git repo.
...
# YOLO mode (no sandbox, fastest)
codex exec --yolo "Fix all lint errors"
```

Key difference: Codex is simpler than Claude Code — fewer flags, less control over permissions. Use `--full-auto` for building, `--yolo` when you trust the outcome. Always use `pty=true` in Hermes terminal calls. Use `--sandbox danger-full-access` when the host service context breaks bubblewrap.

### OpenCode CLI

Install: `npm i -g opencode-ai@latest` or `brew install anomalyco/tap/opencode`. Provider-agnostic (configure via `opencode auth login`).

```bash
# One-shot (no pty needed)
opencode run "Add retry logic to API calls"
opencode run "Review for security issues" -f config.yaml
opencode run "Debug CI failure" --thinking

# Interactive TUI (requires pty=true, background=true)
opencode  # then use process(action='submit') to send prompts
opencode -c  # continue last session
```

Key difference: OpenCode is provider-agnostic (works with OpenRouter, Anthropic, OpenAI, etc.) and open-source. Its `run` command doesn't need PTY. For interactive sessions, use Ctrl+C (not `/exit`) to quit.

### Choosing Between Them

| Criterion | Claude Code | Codex CLI | OpenCode |
|---|---|---|---|
| **Richness** | Highest (permissions, hooks, agents, MCP) | Medium | Medium |
| **Provider lock-in** | Anthropic only | OpenAI only | Provider-agnostic |
| **PTY requirement** | Yes for interactive; `-p` for one-shot | Yes (always) | Only for interactive TUI |
| **Git requirement** | No (but recommended) | **Yes** (refuses without) | Recommended |
| **Best for** | Complex multi-turn work, CI automation | Quick one-shots, scratch work | Mixed-provider setups, PR review |
| **Slash commands** | Rich set (`/review`, `/compact`, etc.) | Minimal | Basic (`Tab` to switch agents) |

**Default choice:** Claude Code for most tasks (richest feature set, best print-mode support). Fall back to Codex if you need OpenAI models or git-enforced scratch work. Use OpenCode if provider flexibility matters or Claude Code isn't installed.
