...
## Phase 0: Check the Power Cord (ALWAYS FIRST)

**Before any analysis, before any code reading, before Phase 1 — exhaust the obvious.**

This phase exists because the most common debugging failure mode is overthinking. You are susceptible to this. You will be tempted to skip this.

### The Power Cord Checklist

Start from the OUTSIDE and work inward. The first thing you reach for should always be the broadest, fastest, dumbest scan:

1. **Grep the config surface.** `grep -r "the-error-string" ~/.config/ ~/.<app>/ /etc/` — the answer is often in a config file, a cron job, a `.bak`, or a `.env`. Do this BEFORE reading source code.
2. **Check the backups and overrides.** `.bak`, `.last-good`, `.env`, `.envrc`, systemd drop-ins, cron jobs, SQLite databases. Override layers are the #1 source of "I changed the config but it keeps reverting."
3. **If the user gives you a direct hint, EXECUTE IT FIRST.** "It's hardcoded somewhere, search for it" means grep, not reverse-engineer the framework. The user knows their system.
4. **Check if it's plugged in.** Is the service running? Is the env var set? Is the API key valid? Is the port open? One command each.
5. **Read the literal error message.** Don't interpret it. Don't theorize. Just read it. Often it says exactly what's wrong.
6. **Has this changed recently?** `git log --oneline -5`, check modified timestamps, check what cron just fired.

### The Power Cord Rule

```
MINIMUM VIABLE INVESTIGATION:
grep -r <error> <config-dir>
         BEFORE
read <source-file>
```

If you catch yourself opening a minified JavaScript file before you've grepped the config directory, stop. You're doing it wrong.

**Joe's Law:** "You don't reinstall the CPU if a computer doesn't turn on. You first make sure it's plugged in."
...
- **Opening minified JS or tracing framework internals before grepping config files**
- **Skipping the user's explicit hint ("it's hardcoded somewhere") to do "deeper analysis"**
...
|-------|---------------|------------------|
| **0. Power Cord** | Grep config surface, check overrides/backups/DBs, verify service is alive, read literal error | Obvious causes eliminated |
