...
- **Verify where the Desktop app runs before modifying any code.** On a remote server, the Electron Desktop app may be running on the user's local machine and connecting to the server's dashboard API. Modifying `web_dist/`, `dist/`, or the ASAR archive on the server will NOT reach the user's Desktop app — those files live inside the Electron bundle on their machine. Before touching any file: `ps aux | grep -i electron` to check if the app runs locally, and ask the user directly ("is the Desktop app on your machine or this server?"). The web dashboard at port 9119 IS served from the server — testing there first confirms whether the fix works before worrying about Desktop deployment.
