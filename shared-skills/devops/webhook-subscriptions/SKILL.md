...
**Important:** Top-level JSON fields use `{fieldname}` directly — NOT
`{payload.fieldname}`. If your POST body is `{"content":"hello"}`,
use `{content}`. `{payload.content}` will NOT be interpolated. Test
with `hermes webhook test <name> --payload '{"content":"test"}'` to
verify template rendering.

**Delivery targets:** `--deliver origin` does NOT work for webhook
subscriptions — responses are sent back to the webhook caller (the void).
Use explicit delivery targets like `--deliver telegram --deliver-chat-id <id>`.
For webhook-to-Telegram relay, this is the only working pattern.
