...
Paul's vault at `/root/.hermes/` (config, identity, skills, profiles) is tracked by a local git repo at `/root/.hermes/.git`.

**First-time setup checklist:**

1. **`.gitignore`** — without one, the vault repo tracks WhatsApp crypto keys, SQLite WALs, logs, cache, audio, and LSP node_modules. See `hermes-maintenance` skill → `references/vault-git-hygiene.md` for the complete template.
2. **Verify tracking:** `git ls-files | grep -E 'whatsapp/|\.db-wal|audio_cache/|logs/'` — must return nothing.
3. **Remote:** `git remote -v` must show a destination. If empty: `git remote add origin git@github.com:OWNER/REPO.git`
4. **SSH key:** generate deploy key: `ssh-keygen -t ed25519 -f /root/.ssh/paul_vps -N "" -C "paul-vps-deploy"`, add to repo via GitHub UI, configure: `git config core.sshCommand "ssh -i /root/.ssh/paul_vps -o StrictHostKeyChecking=accept-new"`
5. **Push:** `git push -u origin master`

**PAT warning:** GitHub fine-grained PATs do NOT work for `git push` over HTTPS. Always use SSH deploy keys.

**Ongoing:** Commit after each meaningful session. `.env` files should NOT be tracked — use `.env.example` for setup reference.
...
When Paul produces a Design Bible explorer as a self-contained HTML file, it appears under "file" — clickable, openable.
